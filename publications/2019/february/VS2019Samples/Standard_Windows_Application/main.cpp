...
Here we declared an example of data struct.
The AlignmentProperties() function shows some properties of the aligment for an instance of the data struct.

*/
struct  _Special_Data_For_Aligment {
...
void AligmentProperties( void ) {

	constexpr uint32_t sizeOfStructInBytes { sizeof( Alignment_Example ) };

	constexpr uint32_t alignmentSize { alignment_of<_Special_Data_For_Aligment>::value };

	_Special_Data_For_Aligment* startAddress { ( addressof< _Special_Data_For_Aligment>( Alignment_Example ) ) };

	_Special_Data_For_Aligment* endAddress { ( addressof< _Special_Data_For_Aligment>( Alignment_Example ) + ( sizeOfStructInBytes - 1ui32 ) ) };
...
int APIENTRY wWinMain( _In_ HINSTANCE hThisInstance, _In_opt_ HINSTANCE  hPrevInstance, _In_ LPWSTR szCmdLine, _In_ int iCmdShow ) {
	...
	PtrToMemoryBasicInformation ptrToMemBasicInfo {
		...
		reinterpret_cast < PtrToMemoryBasicInformation >( HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof( MemoryBasicInformation ) ) )
	...
	*/
	...
	-> remove from memory blocks information that can potentially be security critical, such as Ids, names, cryptographic related data and so on.
